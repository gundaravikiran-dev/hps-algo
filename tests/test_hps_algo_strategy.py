...
class FakeKite:
    ...
    def historical_data(self, instrument_token, from_date, to_date, interval) -> list[dict]:
        if instrument_token == 1:
            return [{"close": 100 + index, "high": 370 if index == 180 else 100 + index} for index in range(230)]
...
def test_find_stocks_above_200_ema(tmp_path: Path) -> None:
    ...
    for index in range(210):
        ...
        aaa_high = 360 if index == 180 else 100 + index
        rows.append(f"AAA,{day},100,{aaa_high},90,{100 + index},1000")
        rows.append(f"BBB,{day},100,{300 - index},90,{300 - index},1000")
...
def test_find_kite_stocks_ltp_above_200_ema() -> None:
    ...
    assert results[0].condition == "EMA10 > EMA20"
    assert "Near EMA10" in results[0].entry_zone
    assert results[0].high_reference == "52W High"
    assert results[0].rsi_14 > 63
...
def test_rejects_when_short_emas_are_below_ema_200(tmp_path: Path) -> None:
    ...
    for index in range(210):
        ...
        rows.append(f"AAA,2026-01-{(index % 28) + 1:02d},100,{close},90,{close},1000")
    csv_path.write_text("\n".join(rows), encoding="utf-8")
